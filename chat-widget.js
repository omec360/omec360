...
function createChatWidget() {
    ...
    widget.innerHTML = `
        ...
        <button id="chat-toggle" onclick="toggleChat()" aria-label="פתח צ'אט עוזר עומק" aria-expanded="false"
            style="position:fixed;bottom:24px;right:24px;z-index:1000;width:60px;height:60px;border-radius:50%;background:#f4c025;border:none;cursor:pointer;box-shadow:0 4px 20px rgba(244,192,37,0.4);display:flex;align-items:center;justify-content:center;transition:transform 0.2s;"
            onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#000" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div id="chat-badge" style="position:fixed;bottom:74px;right:20px;z-index:1000;background:#f4c025;color:#000;font-size:10px;font-weight:800;padding:2px 7px;border-radius:20px;font-family:'Assistant',sans-serif;letter-spacing:0.05em;pointer-events:none;">AI</div>
        ...
        <div id="chat-modal" style="display:none;position:fixed;bottom:100px;right:24px;z-index:999;width:360px;max-width:calc(100vw - 48px);background:#111113;border:1px solid rgba(244,192,37,0.3);border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.6);font-family:'Assistant',sans-serif;direction:rtl;">
            <!-- Header -->
            <div style="background:#f4c025;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <div style="width:36px;height:36px;border-radius:50%;background:#000;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;">ע</div>
                    <div>
                        <div style="font-weight:800;font-size:15px;color:#000;">עוזר עומק</div>
                        <div style="font-size:11px;color:#333;">מבוסס AI · זמין 24/7</div>
                    </div>
                </div>
                <button onclick="toggleChat()" aria-label="סגור צ'אט" style="background:none;border:none;cursor:pointer;font-size:22px;color:#000;line-height:1;">×</button>
            </div>
            ...
            <div style="padding:12px 16px;border-top:1px solid rgba(255,255,255,0.1);display:flex;gap:8px;align-items:flex-end;">
                ...
                <button onclick="sendMessage()" id="send-btn"
                    style="background:#f4c025;border:none;border-radius:10px;width:40px;height:40px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:opacity 0.2s;">
...
function addMessage(text, isUser) {
    ...
    msg.style.cssText = isUser
        ? 'background:#f4c025;color:#000;border-radius:14px 14px 4px 14px;padding:10px 14px;font-size:14px;line-height:1.6;max-width:85%;align-self:flex-end;font-weight:600;'
